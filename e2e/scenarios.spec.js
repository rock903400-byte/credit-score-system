...
test.describe('場景 1-6：常見優質案件', () => {
    ...
    test('① A 級模範生：公務人員 30 歲借 30 萬/5 年', async ({ page }) => {
        await page.goto('/');
        await fillForm(page, { income: 50000, age: 30, existing_debt: 0, loan: 300000, years: 5, rate: 3, shares: 100000 });
        // 公教人員 + 5 年以上年資
        ...
        console.log(`  → 等級 ${grade}，狀態：${status.replace(/\s+/g, ' ').slice(0, 30)}`);
    ...
    test('② 7 年紅線：年限 8 年時系統強制鎖定不動產', async ({ page }) => {
        await page.goto('/');
        await fillForm(page, { income: 45000, age: 35, existing_debt: 0, loan: 500000, years: 8, rate: 3, shares: 100000 });
        ...
        await page.evaluate(() => { document.getElementById('collateral').value = '5'; });
        await page.locator('#btnCalc').click();
        const status = await page.locator('#resStatus').innerText();
    ...
    test('③ DTI 爆表：月入 5 萬但已欠 4 萬月付', async ({ page }) => {
        await page.goto('/');
        await fillForm(page, { income: 50000, age: 40, existing_debt: 40000, loan: 200000, years: 5, rate: 3, shares: 50000 });
        await page.locator('#btnCalc').click();

        const dti = await page.locator('#resTotalDti').innerText();
        const maxDti = await page.locator('#resMaxDtiTxt').innerText();
        const status = await page.locator('#resStatus').innerText();
        console.log(`  → DTI ${dti}% vs 上限 ${maxDti}%；狀態：${status.replace(/\s+/g, ' ').slice(0, 40)}`);
    ...
    test('④ 70 歲借 5 年：到期 75 觸及 >70 閾值，扣 10 分', async ({ page }) => {
        await page.goto('/');
        await fillForm(page, { income: 40000, age: 70, existing_debt: 0, loan: 200000, years: 5, rate: 3, shares: 50000 });
        await page.locator('#btnCalc').click();
    ...
    test('④b 68 歲借 1 年：到期 69，僅觸及 >65，扣 5 分', async ({ page }) => {
        await page.goto('/');
        await fillForm(page, { income: 40000, age: 68, existing_debt: 0, loan: 200000, years: 1, rate: 3, shares: 50000 });
        await page.locator('#btnCalc').click();
    ...
    test('⑤ 76 歲借 3 年：到期 79 超過 75 硬上限', async ({ page }) => {
        await page.goto('/');
        await fillForm(page, { income: 35000, age: 76, existing_debt: 0, loan: 100000, years: 3, rate: 3, shares: 50000 });
        await page.locator('#btnCalc').click();
    ...
    test('⑥ 17 歲未成年：欄位紅框 + 法定代理人同意訊息', async ({ page }) => {
        await page.goto('/');
        await fillForm(page, { income: 25000, age: 17, existing_debt: 0, loan: 30000, years: 2, rate: 3, shares: 50000 });
        await page.locator('#btnCalc').click();
        ...
        const errMsg = await ageField.locator('xpath=ancestor::div[contains(@class,"form-group")]').locator('.error-msg').innerText();
...
test.describe('場景 7-12：法規紅線', () => {
    ...
    test('⑦ 聯徵嚴重瑕疵：直接否決', async ({ page }) => {
        await page.goto('/');
        await fillForm(page, { income: 50000, age: 40, existing_debt: 0, loan: 100000, years: 5, rate: 3, shares: 50000 });
    ...
    test('⑨ 30 年上限：35 年即使有不動產也否決', async ({ page }) => {
        await page.goto('/');
        await fillForm(page, { income: 80000, age: 35, existing_debt: 0, loan: 1000000, years: 35, rate: 3, shares: 100000 });
        // 35 年已自動切到不動產
    ...
    test('⑩ 擔保放款鑑價過期：15 年須重鑑', async ({ page }) => {
        await page.goto('/');
        await fillForm(page, { income: 80000, age: 35, existing_debt: 0, loan: 1000000, years: 20, rate: 3, shares: 100000 });
        await page.locator('#collateral').selectOption('10');
        await page.locator('#appraisalValue').fill('15000000');
    ...
    test('⑪ 屋齡超限：25 年屋齡借 25 年', async ({ page }) => {
        await page.goto('/');
        await fillForm(page, { income: 80000, age: 35, existing_debt: 0, loan: 1000000, years: 25, rate: 3, shares: 100000 });
        await page.locator('#collateral').selectOption('10');
        await page.locator('#appraisalValue').fill('15000000');
    ...
    test('⑫ 抵押權設定 120% 不足：放款接近鑑價上限時', async ({ page }) => {
        ...
        await fillForm(page, { income: 80000, age: 35, existing_debt: 0, loan: 6000000, years: 20, rate: 3, shares: 100000 });
        await page.locator('#collateral').selectOption('10');
...
test.describe('場景 13-18：實務進階', () => {
    ...
    test('⑬ 5 個非社員 vs 5 個社員保證人：社員加分多', async ({ page }) => {
        // 第一次：全非社員
        await page.goto('/');
        await fillForm(page, { income: 50000, age: 30, existing_debt: 5000, loan: 300000, years: 5, rate: 3, shares: 100000 });
        ...
        await page.locator('#btnCalc').click();
        const nonMemberProtection = await page.locator('#bd_protection_val').innerText();
        const nonMemberScore = await page.locator('#gaugeScoreVal').textContent();
        console.log(`  → 非社員：保障 ${nonMemberProtection} 分，總分 ${nonMemberScore}`);
        ...
        await page.locator('#btnCalc').click();
        const memberProtection = await page.locator('#bd_protection_val').innerText();
        const memberScore = await page.locator('#gaugeScoreVal').textContent();
        console.log(`  → 社員：  保障 ${memberProtection} 分，總分 ${memberScore}`);

        expect(parseInt(memberProtection)).toBeGreaterThanOrEqual(parseInt(nonMemberProtection));
        expect(parseInt(memberScore)).toBeGreaterThanOrEqual(parseInt(nonMemberScore));
    });

    test('⑭ 整併貸款：兩筆既有整合 + 月省金額', async ({ page }) => {
        await page.goto('/');
        await fillForm(page, { income: 60000, age: 35, existing_debt: 0, loan: 800000, years: 7, rate: 3, shares: 100000 });
    ...
    test('⑮ 建議增貸額度：股金 2 倍內 (collateral=5) 顯示', async ({ page }) => {
        ...
        await fillForm(page, { income: 100000, age: 35, existing_debt: 0, loan: 500000, years: 5, rate: 3, shares: 200000 });
        ...
        console.log(`  → 等級 ${grade}，建議盒：${text.replace(/\n/g, ' | ').slice(0, 60)}`);
    ...
    test('⑯ 列印報表：社員編號、日期、Report ID 都正確', async ({ page }) => {
        ...
        await fillForm(page, { income: 50000, age: 40, existing_debt: 0, loan: 200000, years: 5, rate: 3, shares: 80000 });
        await page.locator('#btnCalc').click();
        ...
        console.log(`  → 社員：${member} / 等級 ${grade} / ${score} 分 / 額度 ${limit} 元`);
    ...
    test('⑰ 股金矛盾提示：申請 50 萬/股金 10 萬/選「足額股金內」', async ({ page }) => {
        await page.goto('/');
        await fillForm(page, { income: 50000, age: 40, existing_debt: 0, loan: 500000, years: 5, rate: 3, shares: 100000 });
    ...
    test('⑱ 估算小幫手：房貸 500 萬/2.5%/20 年 → 月付約 2.65 萬', async ({ page }) => {
        await page.goto('/');
