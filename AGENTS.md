...
- Unit tests: `node test.js` (runs core.js in a VM, no DOM).
- E2E tests: `npx playwright test`. First time run `npx playwright install chromium`.
...
- Default `<option>` for `#collateral` is `'12'`. If `proposedLoan > shares` you must set collateral to `'5'` or `'10'` to avoid silent veto.
- `pmt()` is exported as a global; do not shadow it.
...
- After filling inputs, wait a bit (`await page.waitForTimeout(100)`) so the debounced localStorage draft saves before reloading.
